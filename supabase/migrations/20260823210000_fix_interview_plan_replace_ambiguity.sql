...
begin
  ...
  if not exists (select 1 from public.phase1_requisitions pr where pr.id = p_requisition_id) then
  ...
  for v_round in select value from jsonb_array_elements(p_rounds) loop
    ...
    values (v_plan_id, v_round->>'stage', btrim(v_round->>'title'), v_round_order)
    ...
    for v_question in select value from jsonb_array_elements(coalesce(v_round->'questions', '[]'::jsonb)) loop
      ...
      insert into public.phase1_interview_questions (round_id, source_id, question_text, areas, sort_order)
      values (
        v_round_id,
