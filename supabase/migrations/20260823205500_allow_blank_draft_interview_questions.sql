-- Blank interview questions are a valid drafting state in the Interview Plan UI.
-- Completion/submission validation will enforce nonblank questions later.

alter table public.phase1_interview_questions
  drop constraint if exists phase1_interview_questions_question_text_check;
...
begin
  ...
  loop
    ...
    loop
      if length(coalesce(v_question->>'text', '')) > 1000 then
