...
import { TrashBin, type TrashedCandidate } from '@/components/TrashBin';
...
export default async function RequisitionPage({params}:{params:{id:string}}){
  ...
  const {data:trashed}=await supabaseAdmin.from('phase1_candidates').select('id,full_name,deleted_at').eq('requisition_id',params.id).not('deleted_at','is',null).order('deleted_at',{ascending:false});
  ...
  const trashedCandidates:TrashedCandidate[]=(trashed||[]).map(c=>({id:c.id,name:c.full_name,deletedAt:c.deleted_at as string}));

  const requisitionView = <section><h2>Job Description</h2><div className="jd">{req.job_description}</div></section>;
  const candidatesView = <><ResumeUpload requisitionId={req.id}/><div className="matrix-header-row"><h2>Candidate Matrix</h2><TrashBin candidates={trashedCandidates}/></div><p className="muted">Compare evaluated candidates - click a name to expand the full assessment.</p><CandidateMatrix candidates={matrixCandidates} positionTitle={req.title}/></>;
