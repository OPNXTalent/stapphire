...
export default async function RequisitionPage({params}:{params:{id:string}}){
  ...
  const matrixCandidates:MatrixCandidate[]=(candidates||[]).map(candidate=>{const evaluations=((candidate.phase1_evaluations as unknown as EvaluationRow[])||[]).sort((a,b)=>b.created_at.localeCompare(a.created_at));const evaluation=evaluations[0];return{id:candidate.id,name:candidate.full_name,sourceFilename:String(candidate.source_filename||''),resumeAvailable:Boolean(candidate.source_storage_path),match:number(evaluation?.overall_match),rankOrder:number(candidate.rank_order),createdAt:String(candidate.created_at),evaluationDate:String(evaluation?.created_at||candidate.created_at),responsibilities:number(evaluation?.job_responsibilities_score),hardSkills:number(evaluation?.hard_skills_score),softSkills:number(evaluation?.soft_skills_score),keywords:number(evaluation?.keyword_terminology_score),assessment:evaluation?.assessment??null,disposition:(['screen','interview','hire','delete'].includes(String(candidate.disposition))?candidate.disposition as Disposition:null)}});
