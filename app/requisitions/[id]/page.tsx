...
type EvaluationRow = {
  ...
  job_responsibilities_score: unknown;
  hard_skills_score: unknown;
  soft_skills_score: unknown;
  keyword_terminology_score: unknown;
  assessment: unknown;
  evaluation_basis_id: unknown;
  created_at: string;
};

type InterviewSubmissionRow = {
  candidate_id: string;
...
function criteriaRollup(assessment: unknown, category: string): number | null {
  const source = record(assessment);
  if (source.evaluation_format !== 'criteria_v1') return null;
  return number(record(source.category_rollups)[category]);
}

function sourceIsNewer(sourceTimestamp: unknown, analysisTimestamp: unknown): boolean {
  if (!sourceTimestamp || !analysisTimestamp) return false;
...
function buildInterviewAggregates(rows: InterviewSubmissionRow[]): Map<string, InterviewAggregate> {
  ...
  for (const row of rows) {
    const current = working.get(row.candidate_id) ?? { total: 0, ratingCount: 0, submitted: 0 };
    current.submitted += 1;

    const ratings = record(record(row.submission_payload).ratings);
    for (const rawValue of Object.values(ratings)) {
    ...
    working.set(row.candidate_id, current);
  }

  return new Map(
    Array.from(working.entries()).map(([candidateId, aggregate]) => [
      candidateId,
      {
        score: aggregate.ratingCount > 0
          ? Math.round((aggregate.total / aggregate.ratingCount) * 100) / 100
          : null,
...
export default async function RequisitionPage({ params }: { params: { id: string } }) {
  ...
  const [
    ...
    { data: dnsList },
    { data: submittedInterviews, error: submittedInterviewsError }
  ] = await Promise.all([
    supabaseAdmin
    ...
    supabaseAdmin
      ...
      .select('candidate_id,submission_payload')
      .eq('requisition_id', params.id)
      .eq('status', 'submitted')
  ]);

  if (candidatesError) throw new Error(`Unable to load candidates: ${candidatesError.message}`);
  if (submittedInterviewsError) throw new Error(`Unable to load interview results: ${submittedInterviewsError.message}`);

  const hiringCriteria = await getHiringCriteriaModel(params.id);
  const interviewAggregates = buildInterviewAggregates((submittedInterviews ?? []) as InterviewSubmissionRow[]);

  const matrixCandidates: MatrixCandidate[] = (candidates || []).map((candidate) => {
    const evaluations = ((candidate.phase1_evaluations as unknown as EvaluationRow[]) || [])
    ...
    return {
      ...
      match: number(evaluation?.overall_match),
      rankOrder: number(candidate.rank_order),
      createdAt: String(candidate.created_at),
      evaluationDate: String(evaluation?.created_at || candidate.created_at),
      evaluationBasisId: typeof evaluation?.evaluation_basis_id === 'string' ? evaluation.evaluation_basis_id : null,
      responsibilities: number(evaluation?.job_responsibilities_score),
      hardSkills: number(evaluation?.hard_skills_score),
      softSkills: number(evaluation?.soft_skills_score),
      keywords: number(evaluation?.keyword_terminology_score),
      otherRequirements: criteriaRollup(assessment, 'other_requirements'),
      assessment,
      disposition: ['screen', 'interview', 'hire', 'delete'].includes(String(candidate.disposition))
  ...
  const dnsCandidates: DNSCandidate[] = (dnsList || []).map((candidate) => ({
    id: candidate.id,
    name: candidate.full_name,
    deletedAt: candidate.deleted_at as string
  }));

  const hiringCriteriaView = (
    <HiringCriteria
      model={hiringCriteria}
      requisitionId={req.id}
      sourceIsStale={sourceIsNewer(req.job_description_updated_at, hiringCriteria?.generatedAt)}
    />
  );
  ...
  const interviewsView = (
    <InterviewPlan
      ...
      candidateNames={matrixCandidates.map((candidate) => candidate.name)}
    />
  );

  const candidatesView = (
    <CandidateMatrix candidates={matrixCandidates} positionTitle={req.title} requisitionId={req.id} />
  );

  return (
    <RequisitionViewToggle
      title={req.title}
      requisitionId={req.id}
