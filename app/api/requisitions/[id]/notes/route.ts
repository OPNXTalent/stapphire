...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from('phase1_requisition_notes')
      .insert({ requisition_id: params.id, author_name: authorName, body: noteBody })
      .select('id, author_name, body, created_at')
      .single();
    if (error) throw error;
    ...
    console.error(error);
    return NextResponse.json({ error: 'Unable to post note.' }, { status: 500 });
  }
}
