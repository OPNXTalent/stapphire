...
function normalizeAssessment(value:unknown){
  ...
  return {assessment:text(source.assessment),standoutReasons:texts(source.standout_reasons),strongestMatches,mostImportantConcern:text(source.most_important_concern),dealBreakers:texts(source.deal_breakers),whatToVerify:texts(source.what_to_verify),trainableAfterHire:texts(source.trainable_after_hire),ats:{level:text(ats.level),reasoning:text(ats.reasoning)},employment:{previousTransitEmployer:text(employment.previous_transit_employer),gaps:texts(employment.gaps),shortTenure:texts(employment.short_tenure),stability:text(employment.stability)},strategicRisk:text(source.strategic_risk),interviewPriorities:texts(source.interview_priorities),finalRecommendationReasoning:text(source.final_recommendation_reasoning)};
}
...
export default async function CandidatePage({params}:{params:{id:string}}){
  ...
  {(a.employment.previousTransitEmployer||a.employment.gaps.length>0||a.employment.shortTenure.length>0||a.employment.stability)&&<><h2>Employment History Review</h2><p><strong>Previous Transit Employer:</strong> {a.employment.previousTransitEmployer||'None Identified'}</p>{a.employment.gaps.length>0&&<><h3>Gaps</h3><List items={a.employment.gaps}/></>}{a.employment.shortTenure.length>0&&<><h3>Short tenure</h3><List items={a.employment.shortTenure}/></>}{a.employment.stability&&<><h3>Stability</h3><p>{a.employment.stability}</p></>}</>}
