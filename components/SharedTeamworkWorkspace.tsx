...
const prospectScore = (prospect: Record<string, unknown>) => Number(prospect.evaluation_score ?? prospect.preliminary_score);
const prospectFit = (prospect: Record<string, unknown>) => {
  const score = prospectScore(prospect);
  return score >= 80 ? 'strong evidence' : score >= 70 ? 'potential fit' : 'weak fit';
};
