...
export function AppShell({requisitions,children}:{requisitions:RequisitionLink[];children:React.ReactNode}){
  ...
  return <><header className="brand-bar"><a className="brand-home" href="/"><Gem/><span className="brand-word">Stapphire</span></a><span className="brand-tagline">Hiring Quality Control</span><span className="brand-workspace">an OPNX workspace</span></header><div className={`app-shell ${rightCollapsed?'right-collapsed':''}`}><aside className="req-nav"><div className="req-nav-head"><span className="eyebrow">Workspace</span><a className="new-req-link" href="/#new-requisition">+ New requisition</a></div><nav aria-label="Requisitions"><a className={`req-nav-all ${pathname==='/'?'active':''}`} href="/">All requisitions</a><span className="eyebrow req-nav-label">Open requisitions</span>{requisitions.map(req=><a className={`req-nav-item ${pathname===`/requisitions/${req.id}`?'active':''}`} href={`/requisitions/${req.id}`} key={req.id}><span className="req-status-dot"/><span className="req-nav-title">{req.title}</span></a>)}{!requisitions.length&&<p className="req-nav-empty">No requisitions yet</p>}</nav><div className="req-nav-footer">Hiring decisions, grounded in evidence.</div></aside><main className="workspace-main"><div className="workspace-content">{children}</div></main><WorkspacePanel collapsed={rightCollapsed} onExpand={()=>setRightCollapsed(false)} onCollapse={()=>setRightCollapsed(true)} /></div></>;
}
