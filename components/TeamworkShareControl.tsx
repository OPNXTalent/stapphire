...
export function TeamworkShareControl({ requisitionId }: { requisitionId: string }) {
  ...
  return <div className={styles.wrap}>
    ...
    {open&&<section className={styles.panel} aria-label="Teamwork share links"><div className={styles.heading}><div><strong>Invite to this requisition</strong><p>Candidate Files and recruiter notes stay private.</p></div><button type="button" onClick={()=>setOpen(false)} aria-label="Close share access">×</button></div>
      <form onSubmit={create} className={styles.form}><label><span>Invited by</span><input value={inviter} onChange={(event)=>setInviter(event.target.value)} maxLength={80} placeholder="Your name" required/></label><label><span>Access</span><select value={access} onChange={(event)=>setAccess(event.target.value as 'viewer'|'contributor')}><option value="contributor">Contributor — may comment</option><option value="viewer">Viewer — read only</option></select></label><button disabled={busy}>{busy?'Creating…':'Create link'}</button></form>
