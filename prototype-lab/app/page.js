...
export default function Page() {
  ...
  return <main>
    ...
    {result && <section className="output">
      ...
      <section className="marketRead"><header><div><small>TALENT MARKET READ</small><h2>Is this truly a unicorn search?</h2></div><div className={`scarcity ${result.marketAnalysis.scarcityLevel.toLowerCase()}`}><strong>{label(result.marketAnalysis.scarcityLevel)}</strong><span>{label(result.marketAnalysis.confidence)} confidence</span></div></header><p>{result.marketAnalysis.summary}</p><div className="marketStats"><div><strong>{result.marketAnalysis.observedProspects}</strong><span>verified prospects surfaced</span></div><div><strong>{gates.length}</strong><span>non-negotiable gates</span></div><div><strong>{label(searchScope)}</strong><span>search reach</span></div></div><div className="marketColumns"><div><h3>What is constraining the pool</h3>{result.marketAnalysis.constraintDrivers.map((driver) => <article key={`${driver.constraint}-${driver.explanation}`}><header><strong>{driver.constraint}</strong><span>{label(driver.impact)} impact</span></header><p>{driver.explanation}</p></article>)}</div><div><h3>Ways to expand it</h3>{result.marketAnalysis.relaxationLevers.map((lever) => <article key={`${lever.change}-${lever.likelyEffect}`}><strong>{lever.change}</strong><p>{lever.likelyEffect}</p><small>Tradeoff: {lever.tradeoff}</small></article>)}</div></div><footer>{result.marketAnalysis.evidenceCaveat}</footer></section>
      {result.prospects.length === 0 ? <div className="empty">No identity-resolved prospects cleared the current gates and search scope. Review the market analysis before broadening the search.</div> : <><div className="tableHead"><span>Prospect</span><span>Location</span><span>Sourcing fit</span><span>Evidence score</span><span /></div>
