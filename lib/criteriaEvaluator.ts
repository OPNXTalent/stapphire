  ...
  properties: {
    ...
    weighted_criteria: {
      ...
      items: {
        ...
        required: ['criterion_id', 'score', 'evidence', 'assessment'],
        properties: {
          criterion_id: { type: 'string' },
          score: { type: 'integer', enum: CRITERION_SCORES },
    ...
    knockout_criteria: {
      ...
      items: {
        type: 'object', additionalProperties: false,
        required: ['criterion_id', 'status', 'evidence', 'assessment'],
        properties: {
          criterion_id: { type: 'string' },
          status: { type: 'string', enum: KNOCKOUT_STATUSES },
          evidence: { type: 'string' },
          assessment: { type: 'string' }
        }
      }
...
export async function evaluateCandidateAgainstCriteria(jobDescription: string, criteria: AppliedCriterion[], resumeText: string): Promise<CriteriaAwareModelEvaluation> {
  ...
  const response = await openai.responses.create({
    ...
    store: false,
    text: { format: { type: 'json_schema', name: 'criteria_candidate_evaluation', strict: true, schema: criteriaSchema } }
  });
